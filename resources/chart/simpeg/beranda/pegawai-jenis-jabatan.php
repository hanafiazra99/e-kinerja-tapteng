...
$conn = req_data_where($koneksi, 'cv_c_pegawai_jenis_jabatan', $where);

$arr_kategori = array();
$arr_nilai = array();

while ($data = mysqli_fetch_array($conn)) {
    $jenis_jabatan = ($data['jenis_jabatan'] == '' ? 'Tidak Diketahui' : $data['jenis_jabatan']);

    if (!in_array($jenis_jabatan, $arr_kategori)) {
        array_push($arr_kategori, $jenis_jabatan);
        array_push($arr_nilai, 0);
    }

    $key = array_search($jenis_jabatan, $arr_kategori);
