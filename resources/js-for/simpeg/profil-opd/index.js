$(document).ready(function () {
    ...
    var table1 = $('#daftar_data_l1').DataTable({
        ...
        "serverSide": true,
        "ordering": false,
        "ajax": RESOURCES_URL + "tabel/simpeg/profil-opd/unit-organisasi_data.php?opd="+opd,
        lengthMenu: [
            [5, 10, 25, 100, -1],
            [5, 10, 25, 100, "All"]
        ],
        ...
        buttons: [{
                ...
                action: function (e, dt, node, config) {
                    document.location.href = SIMPEG_URL + 'profil-opd/unit-organisasi/tambah/';
                },
    ...
    var table2 = $('#daftar_data_r1').DataTable({
        ...
        "serverSide": true,
        "ordering": false,
        "ajax": RESOURCES_URL + "tabel/simpeg/profil-opd/struktur-organisasi_data.php?opd="+opd,
        lengthMenu: [
            [5, 10, 25, 100, -1],
            [5, 10, 25, 100, "All"]
        ],
        ...
        buttons: [{
                ...
                text: 'Tambah Data',
                action: function (e, dt, node, config) {
                    document.location.href = SIMPEG_URL + 'profil-opd/struktur-organisasi/tambah/';
                },
                init: function (api, node, config) {
                    $(node).removeClass('btn-default')
                }
    ...
    $(document).on('click', 'TombolHapus', function () {
		...
		$.ajax({
			type: "post",
			url: SIMPEG_URL + "controllers/c-profil-opd.php",
			data: {
				req: req,
				judul: judul,
				form: form,
...
function init() {
    var mapOptions = {
        zoom: 17,
        center: new google.maps.LatLng(1.736742, 98.785090), 
    };

    var mapElement = document.getElementById("titik-koordinat");
    var map = new google.maps.Map(mapElement, mapOptions);
    var marker = new google.maps.Marker({
        position: new google.maps.LatLng(kantor_latitude, kantor_longitude),
        map: map,
        title: "APA YA"
    });
    marker.setMap(map);
