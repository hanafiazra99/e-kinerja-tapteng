...
$columns = array(
    ...
    array(
        ...
        'formatter' => function ($d, $row) {
            if ($_GET['status'] == 'Diperiksa') {
                return '<SuntingNilai judul="Sunting Kualitas" formreq="SuntingPenilaianBulanan" parameter_pengenal="id" data_pengenal="' . $row[4] . '" parameter_diganti="kualitas" data_diganti="' . $d . '" fieldreq="Kualitas" style="cursor: pointer; border-bottom: 3px dotted #000;">' . number_format($d, 2, ',', '.') . '</SuntingNilai>';
            } else {
                return number_format($d, 2, ',', '.');
            }
        },
