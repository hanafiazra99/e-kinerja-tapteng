...
if (isset($_POST['TombolTambahIzin'])) {
    ...
    $proses_check = array();
    $masa_waktu = explode("-",$_POST['masa_izin']);
    ...
    array_push($proses_check, submit_data($koneksi, $data, $field, 'absensi_izin'));

    if (!in_array("fail", $proses_check)) {
        pop_up_direct(BASE_URL, ABSENSI_URL . 'laporan/rekap-izin/', 'Berhasil', 'Berhasil mengubah data.', 'modal-success');
    } else {
        pop_up_direct(BASE_URL, ABSENSI_URL . 'laporan/rekap-izin/', 'Gagal', 'Terjadi kesalahan pada sistem, laporkan ini pada pihak pengembang.', 'modal-danger');
    }
