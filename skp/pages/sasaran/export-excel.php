...
$data_ttd = req_get_where($koneksi, 'cv_skp_sasaran_ttd', 'id = "' . $data['id'] . '"');
$conn_kegiatan = req_data_where($koneksi, 'v_skp_sasaran_kegiatan', 'sasaran = "' . $data['id'] . '"');
...
$objPHPExcel->getProperties()->setCreator(BASE_TITLE)
    ...
    ->setTitle("Sasaran Kerja Pegawai Negeri Sipil")
    ->setSubject("Sasaran Kerja Pegawai Negeri Sipil " . $data_ttd['pegawai_nama'] . "")
    ->setDescription("Sasaran Kerja Pegawai Negeri Sipil " . $data_ttd['pegawai_nama'] . " " . content_tgl_indo($data['p_awal']) . ' - ' . content_tgl_indo($data['p_akhir']) . "")
    ->setKeywords("Sasaran Kerja Pegawai Negeri Sipil " . $data_ttd['pegawai_nama'] . " " . content_tgl_indo($data['p_awal']) . ' - ' . content_tgl_indo($data['p_akhir']) . "")
    ->setCategory("Sasaran Kerja Pegawai Negeri Sipil");
...
$objPHPExcel->getActiveSheet(0)
    ->getCell('A3')
    ->setValue(content_tgl_indo($data['p_awal']) . ' - ' . content_tgl_indo($data['p_akhir']));
$objPHPExcel->getActiveSheet(0)
    ->getStyle('A3')
    ->getAlignment()
    ->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
...
while ($data_kegiatan = mysqli_fetch_array($conn_kegiatan)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('K' . $cell_no . '', $data_kegiatan['waktu'])
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $cell_no . '', 'Sibolga, ' . content_tgl_indo($data['p_awal']) . '');
$cell_no++;
$objPHPExcel->setActiveSheetIndex(0)
...
$nama_file = "Sasaran Kerja Pegawai Negeri Sipil " . $data_ttd['pegawai_nama'] . " " . content_tgl_indo($data['p_awal']) . ' - ' . content_tgl_indo($data['p_akhir']) . ".xlsx";
